...
test('metered coverage over a window whose rows were deleted is partial, not complete', () => {
  ...
  assert.equal(whole.coverage, 'complete', 'the baseline: every surviving row priced from a matched card');

  const pruned = meteredClaimSupport({ totalCostUsd: 12, estimatedCostUsd: 0, retention: truncated });
  assert.equal(pruned.coverage, 'partial', 'a window that lost rows is not completely covered by its evidence');
...
test('a deletion does not leak into the monetary basis', () => {
  ...
  const pruned = meteredClaimSupport({ totalCostUsd: 12, estimatedCostUsd: 0, retention: truncated });
  assert.equal(pruned.monetaryBasis, 'list', 'still priced from a matched rate card');
...
test('an empty truncated window does not report its emptiness as the only fact', () => {
  ...
  assert.equal(empty.coverage, 'partial', 'unknown pricing over a window known to have lost rows is not unknown coverage alone');
...
test('with no prune on record nothing is asserted about deletion', () => {
  ...
  assert.equal(support.coverage, 'complete');
...
test('the overview payload carries the window coverage it was computed over', () => {
  ...
  try {
    ...
    assert.equal(before.claimSupport.coverage, 'complete');
    ...
    assert.equal(after.claimSupport.coverage, 'partial', 'the server states it rather than leaving the browser to infer it');
  } finally {
    store.close();
...
test('a window that starts after the boundary is intact even on a pruned ledger', () => {
  ...
  try {
    ...
    assert.equal(today.claimSupport.coverage, 'complete');
  } finally {
    store.close();
    rmSync(dir, { recursive: true, force: true });
  }
