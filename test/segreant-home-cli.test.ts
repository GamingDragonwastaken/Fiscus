 ...
 *
 * `segreant-identity.test.ts` proves `config.ts` ignores the pre-rename variable.
 ...
 * spawns the actual CLI, through `bin/segreant.mjs` and therefore through the
...
const CLI = join(REPO, 'bin', 'segreant.mjs');

/** Every override, so a case can guarantee exactly which names are present. */
const OVERRIDES = [
  'SEGREANT_HOME', 'SEGREANT_DB', 'SEGREANT_DEMO',
...
test('the CLI ignores the pre-rename home variable when it is the only one set', async (t) => {
  const fakeHome = mkdtempSync(join(tmpdir(), 'segreant-fakehome-'));
  const legacy = mkdtempSync(join(tmpdir(), 'segreant-legacy-'));
  t.after(() => {
    for (const d of [fakeHome, legacy]) rmSync(d, { recursive: true, force: true });
  });

  const realHomeBefore = listing(join(homedir(), '.segreant'));
  ...
  // `bin/segreant.mjs` runs the compiled dist/, so this fails if dist is absent
  ...
  assert.equal(code, 0, 'segreant demo should succeed — if this fails, check dist/ is built and no build is running concurrently');
  ...
  assert.ok(
    listing(join(fakeHome, '.segreant')).some((f) => f.startsWith('demo.db')),
    `expected the default home to be seeded; found ${JSON.stringify(listing(join(fakeHome, '.segreant')))}`,
  );
  assert.deepEqual(
    listing(join(homedir(), '.segreant')),
    realHomeBefore,
    'a redirected run must not touch the operator\u2019s real ledger home',
  );
});

test('the CLI honours SEGREANT_HOME over the default home', async (t) => {
  const fakeHome = mkdtempSync(join(tmpdir(), 'segreant-fakehome-'));
  const live = mkdtempSync(join(tmpdir(), 'segreant-live-'));
  t.after(() => {
    for (const d of [fakeHome, live] ) rmSync(d, { recursive: true, force: true });
  });

  const code = await run(['demo'], fakeHome, { SEGREANT_HOME: live });
  assert.equal(code, 0, 'segreant demo should succeed');

  assert.ok(
    listing(live).some((f) => f.startsWith('demo.db')),
    `expected SEGREANT_HOME to be seeded; found ${JSON.stringify(listing(live))}`,
  );
  assert.deepEqual(
    listing(join(fakeHome, '.segreant')),
    [],
    'the default home must be untouched when SEGREANT_HOME is set',
  );
});
