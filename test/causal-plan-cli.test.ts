...
test('causal plan CLI previews a family-wise plan without persisting or opening a study', () => {
  const root = mkdtempSync(join(tmpdir(), 'segreant-causal-plan-cli-'));
  ...
  try {
    ...
    const stdout = execFileSync(process.execPath, ['bin/segreant.mjs', 'causal', 'plan', '--study', 'missing-study', '--options', options, '--json'], {
      cwd: process.cwd(),
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe'],
      env: { ...process.env, SEGREANT_HOME: root },
    });
