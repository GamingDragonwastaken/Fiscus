/**
 * `.fiscuspack` has a producer: the epistemic ledger, through `fiscus pack`.
 *
 ...
 * the manifest instead of dropping records, and signs on request; `fiscus
 * pack export|verify|inspect` is the operator face.
 *
 * Checked here: a round trip through the in-tree verifier AND the standalone
 * verifier (`standalone/fiscuspack-verifier.mjs`, no producer import); every
...
import { generateFiscusPackKeyPair, serializeFiscusPack, verifyFiscusPack } from '../src/pack/index.ts';
...
function record(id: string, sensitivity: EvidenceInput['sensitivity'], value: string) {
  return evidence({
    id, evidenceType: 'ops.feed', sourceIdentity: 'fiscus:local', sourceClass: 'fiscus_local_records',
...
test('the exported pack verifies in-tree and with the standalone verifier, and every reference digest is the stored record', () => {
  ...
  const local = verifyFiscusPack(encoded);
  ...
  const dir = mkdtempSync(join(tmpdir(), 'fiscuspack-ledger-'));
  const file = join(dir, 'ledger.fiscuspack.json');
  writeFileSync(file, encoded);
  const standalone = JSON.parse(execFileSync(process.execPath, [join(import.meta.dirname, '..', 'standalone', 'fiscuspack-verifier.mjs'), file], { encoding: 'utf8' }));
...
test('a signed pack is integrity-verified with the embedded key and authentic only with the trust anchor', () => {
  ...
  const encoded = serializeFiscusPack(pack);
  const embeddedOnly = verifyFiscusPack(encoded);
  ...
  const pinned = verifyFiscusPack(encoded, { trustedPublicKey: keys.publicKey });
  assert.equal(pinned.authenticity, 'verified');
  const wrong = verifyFiscusPack(encoded, { trustedPublicKey: generateFiscusPackKeyPair().publicKey });
