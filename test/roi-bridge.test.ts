...
function makeRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'segreant-bridge-'));
  g(dir, ['init', '-q']);
...
test('store: summary scopes to a project, hasProjectSpend gates, characterization groups', () => {
  ...
  try {
    ...
    store.insertRequest(importedRow('segreant', 'codex', t, 9.0, 'c'));
    ...
    assert.ok(Math.abs(store.summary(start, end, 'segreant').costUsd - 9.0) < 1e-9);
    ...
    assert.equal(store.hasProjectSpend('segreant'), true);
    ...
    assert.deepEqual(ch.byProject.map((b) => b.label).sort(), ['game', 'segreant']);
