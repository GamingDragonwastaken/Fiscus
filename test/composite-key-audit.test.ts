...
const CLASSIFIED: ReadonlyArray<{ file: string; contains: string; reason: Reason; why: string }> = [
  ...
  { file: 'src/connect/codex.ts', contains: 'codex:${sessionId ?? \'unknown\'}:${ordinal++}', reason: 'numeric_tail', why: 'the ordinal is numeric and last; a session id may hold a colon and still parses from the right' },
