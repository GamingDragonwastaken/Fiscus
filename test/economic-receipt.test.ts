...
test('economic receipt v2 carries exact effective Money lineage and verifies semantically', () => {
  const dir = mkdtempSync(join(tmpdir(), 'segreant-economic-receipt-'));
  try {
    ...
    const body = buildEconomicReceiptBody('deadbeef', 'segreant', 1.234567, null, funnel, economic);
...
test('economic receipt v2 refuses incomplete exact coverage and tampered lineage', () => {
  const dir = mkdtempSync(join(tmpdir(), 'segreant-economic-receipt-invalid-'));
  try {
    const keys = loadOrCreateKeyPair(join(dir, 'key.json'));
    ...
    assert.throws(() => buildEconomicReceiptBody('deadbeef', 'segreant', 1, null, funnel, incomplete), /complete|unresolved/i);

    const exact = economicAttributionView({
      amount: money('1', 'USD', 'effective'), eventIds: ['economic:request:r1:charge'],
      sourceBases: ['list'], requestCount: 1, unresolvedRequests: 0,
    });
    const signed = signReceipt(buildEconomicReceiptBody('deadbeef', 'segreant', 1, null, funnel, exact), keys);
