/**
 * The team server's HTTP surface. Unlike Segreant's own dashboard
...
export function createTeamServer(deps: TeamServerDeps): http.Server {
  ...
  return http.createServer((req, res) => {
    ...
    if (url.pathname === '/health') {
      return json(res, 200, { ok: true, service: 'segreant-team-server' });
    }
    ...
    if (url.pathname === '/rollups') {
      ...
      runAsyncRoute(res, async () => {
        ...
        if (!developer) {
          return json(res, 403, { ok: false, error: 'unregistered key — ask a team admin to register your `segreant team push --pubkey` output first' });
        }
