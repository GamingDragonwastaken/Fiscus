...
| Last CI-verified exact head | `aa5ec2271132336e30e6968d12eeb54b2aa9cb19` — run `34174439515`, **success on all eight configured jobs** (`test` ×3, `team-server-test` ×3, `package-smoke`, `candidate-head`), inspected job by job, not by trusting the first green row. `2aa7cac` (run `34174382288`), `51f3ec6` (run `34174019795`), `0f0811d` (run `34173907843`), `0ebb966` (run `34173069140`), `e0cd80f` (run `34172882201`), `dfcfc87` (run `34171128690`), `ac98395` (run `34170679364`) and `6b91318` (run `34169977899`) were each verified the same way before it. |
| Code ahead of that head | `99e4384`, a cherry-pick of the owner's `1572f54` from `main`, plus this records commit. Not verified remotely until its own exact SHA has a completed green run. |
...
3. **`latestAnalysis` on the dashboard study rows is still a bare null.** The same defect D-158 fixed on the CLI, on the other surface. It was left because the payload contract change belongs with the version-2 analysis projection rather than ahead of it.
4. **AII-036's three `unmigrated_authority` boundaries** — `causal.qualification`, `causal.estimate`, `decision.certificate`. The first two are in the product import closure and can reach an operator today, which sets the order.
