...
| AII-017 | P0 | Money not exact end-to-end | PARTIAL | Provider/local/reconciliation kernel paths, bundled exact decimal pricing, live proxy/import exact request issuance, complete-coverage exact budget enforcement, schema-owned exact allocation projection/persistence and exact economic export now use exact Money and canonical coefficients; migrate refreshed-card provenance and legacy request/DB/value/API authoritative paths and remove accounting-number authority |
