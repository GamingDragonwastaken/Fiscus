...
| `WP-R05` | Trust Non-Escalation | `PARTIAL` | Direct Claim persistence now enforces weakest-cited-evidence ceilings for integrity and authenticity at the unavoidable `EpistemicLedger.appendClaim` boundary (uncommitted current work; five adversarial tests pass). Coverage, measurement, causality, finality, decision-fitness and witness-mediated derivation paths remain governed by the existing derivation checker or are not yet bounded for direct claims. |
