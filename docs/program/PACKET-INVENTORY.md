...
| `WP-B01` | Universal issuance legality at claim-strengthening boundaries | `PARTIAL` | `causal.issuance` and the new `decision.epistemic` adapter bind causal and strict interval-dominance claim strengthening to kernel Evidence/Witness/Derivation records. Decision issuance is still unreached by a product consumer, and other high-consequence control/policy boundaries remain to inventory. |
...
| `WP-R03` | No Granularity Laundering | `PARTIAL` | `EpistemicLedger` enforces direct claim grain at the persistence boundary: finer and incomparable claims are refused unless an explicit billing aggregate adapter supplies the cross-grain bridge; equal/coarser claims remain admissible. Seven RED-first tests cover refusal, cited-grain supply, coarsening, and the billing compatibility exception (D-106). Remaining: grain is compared by dimension-set containment only, so nothing checks that a dimension name denotes the same partition in two records; scope is not bounded the same way; and derivation grain remains governed by its separate witness requirement. |
