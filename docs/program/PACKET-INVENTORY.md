...
| `WP-B01` | Universal issuance legality at claim-strengthening boundaries | `PARTIAL` | `causal.issuance` and `decision.epistemic` bind causal and strict interval-dominance claim strengthening to kernel records. Budget enforcement now also carries an explicit projection basis and, for incomplete exact coverage, enforces the larger exact-resolved/legacy estimate figure rather than failing open (D-107). Decision issuance remains unreached by a product consumer; other high-consequence control/policy boundaries remain to inventory. |
...
| `WP-R03` | No Granularity Laundering | `PARTIAL` | The direct claim boundary now refuses finer and incomparable grains when no cited evidence supplies the declared dimensions; equal/coarser claims remain admissible, and product rollups retain explicit typed bridges. RED-first tests cover refusal, cited-grain supply, coarsening, and compatibility preservation (D-106). Remaining: dimension names have no partition hierarchy, scope is not bounded equivalently, and derivation grain remains governed by its witness requirement. |
| `WP-R04` | Negative-Claim Soundness | `PARTIAL` | The reachable coding-realization negative `clean` predicate is withheld from kernel issuance when modern exact/mature/realized snapshots lack a qualifying `CompletenessWitness` for both `commit_reverted` and `linked_incident`; compatibility snapshot persistence remains intact. RED-first coverage proves no kernel Claim is issued and valid complete persistence remains idempotent. Reconciliation's residual/off-path bound is already typed and carried through its Claim. Remaining: other negative claims need the same boundary audit; local git source only witnesses revert coverage, so coding `clean` remains unresolved without an incident-feed witness. |
