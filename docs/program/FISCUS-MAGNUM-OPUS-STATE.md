...
- [x] Local verification at this candidate: 1,058 total root tests, 1,054 pass, 0 fail, 4 platform-conditional skips; root/browser typechecks, build, focused economic/request/correction/FX/reprice/allocation tests, and isolated-cache package dry-run pass.

This is a local candidate, not a remote or release checkpoint. Legacy `REAL` request/value/export consumers remain compatibility projections for now, and event-role/conservation, FX consumer integration beyond source-bound translation, persisted allocation, and close semantics remain open. The economic ledger has an operator inspection command but no `/api/economic` or dashboard surface yet. GitHub publication and exact-SHA CI are external gates currently blocked by the Codex usage/approval limit.
...
1. Persist exact allocation with immutable source lineage, then migrate value/export consumers onto the effective economic projection, add period close semantics and the API/dashboard contract. The local candidate now has bundled canonical exact rate source, live proxy/import issuance, exact budget enforcement under complete coverage, CLI inspection, bounded correction and historical FX lineage, legacy-reprice convergence, and a read-only exact allocation projection, but refreshed-card provenance, effective-projection coverage beyond requests, persisted allocation, API/dashboard exposure and legacy value/export read-model migration remain.
