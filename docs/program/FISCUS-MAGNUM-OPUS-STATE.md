...
- Local candidate verification: 1,053 root tests total (1,049 pass, 0 fail, 4 platform-conditional skips), root/browser typecheck, build, and isolated-cache package dry-run pass. No GitHub Actions run exists for these local-only commits because the push was refused by the Codex usage/approval limit.
...
- [x] Local verification at this candidate: 1,053 total root tests, 1,049 pass, 0 fail, 4 platform-conditional skips; root/browser typechecks, build, focused economic/request/correction/FX/reprice tests, and isolated-cache package dry-run pass.

This is a local candidate, not a remote or release checkpoint. Legacy `REAL` request/allocation/value/export consumers remain compatibility projections for now, and event-role/conservation, FX consumer integration beyond source-bound translation, allocation, and close semantics remain open. The economic ledger has an operator inspection command but no `/api/economic` or dashboard surface yet. GitHub publication and exact-SHA CI are external gates currently blocked by the Codex usage/approval limit.
...
1. Migrate allocation/value/export consumers onto the effective economic projection, then add period close semantics and the API/dashboard contract. The local candidate now has bundled canonical exact rate source, live proxy/import issuance, exact budget enforcement under complete coverage, CLI inspection, bounded correction and historical FX lineage, and legacy-reprice convergence, but refreshed-card provenance, effective-projection coverage beyond requests, API/dashboard exposure and legacy allocation/value/export read-model migration remain.
