...
has generated nested dashboard-interface metadata `aad39bc`, the close-to-kernel
bridge `e2ea654`, exact allocation-to-kernel issuance `1428a77`, strict
coding-realization-to-kernel issuance `ad74ea3`, canonical shared dashboard
types `e00f7f9`, named JSON response contracts `cfb1fc7`, and typed realization
...
descriptor and regenerates the browser copy under the publication lock.
`0467830` adds an immutable CapabilitySpec for every GUI capability with
...
The canonical named type source and JSON route identifiers now exist; opaque
nested detail schemas plus full docs/claim/egress generation remain.
...
| AII-018 analytical economic subledger absent | PARTIAL | Immutable typed economic events, canonical exact pricing, role-aware append-only projections, one-source additive local price-correction lineage with multi-hop allocation root resolution, historical exact FX translation with persisted local observations and effective/as-of selection, effective request-charge projection, exact allocation persistence, exact economic export, `/api/economic`, and digest-bound period-close/reopen controls now exist as a local candidate; complete event-role/conservation semantics, provider-authoritative FX policy, FX consumer integration beyond the bounded Store/export/dashboard/CLI seams, value/dashboard consumption beyond bounded seams, and universal product integration remain. |
...
**Dashboard contract addendum (2026-08-30):** `e244115` extends the
existing seeded endpoint conformance check from required-field presence to
runtime primitive, array, record, named-interface and null-union validation.
...
an immutable, conservative CapabilitySpec, and every dashboard API method has
a shared top-level JSON/text envelope contract copied into the browser build.
The modern client rejects a wrong or missing envelope with a typed boundary
error; seeded conformance covers named browser interfaces plus all remaining
...
source and generating docs/claim/egress contracts remain open. The close-kernel
bridge at `e2ea654` revalidates an active finalized period and issues one exact,
idempotent Evidence/Claim pair carrying the close digest, source-event set and
basis-separated balances. The exact-allocation kernel bridge at `1428a77` is
deliberately narrower than allocation completion: after a persisted exact run
is reloaded and its digest, lineage, conservation and coverage are revalidated,
the Store issues one idempotent `economic.allocation_recorded` Evidence/Claim
pair. The claim is a provisional, self-authenticated allocated-showback
statement; it preserves currency/basis groups, source-event IDs, unresolved
legacy IDs and the exact result payload, and partial runs remain partial. It
does not assert provider billing, settlement, chargeback, causal value or an
allocation-specific closed period. Binding allocation execution to the
economic close lifecycle remains a separate control migration; value, billing,
decision and control boundaries still require migration.
...
The named-response follow-on at `cfb1fc7` replaces the remaining inline JSON
route descriptions with explicit shared interfaces for importers, discovery,
scan setup, pricing, realization, guide, judge and proposal clearing. The
browser write methods consume the same names; text/CSV and opaque nested
realization/report details remain intentionally bounded. The nested realization
detail checkpoint at `2f1e509` then names the eight-gate results,
maturity/currentness, waste buckets, serial bounds, unit lineage and
exact-economic coverage rather than leaving the report as an opaque record. The
next contract work is generation of documentation, claim and egress bindings
from this source.

**Execution-order override after `2f1e509`:** the versioned exact
...
bridge at `ad74ea3`, the canonical named dashboard type source at `e00f7f9`,
and named JSON response contracts at `cfb1fc7`, plus typed realization detail at
