...
(`buildRollupBody`/`signRollup`/`verifyRollup`) and the `segreant team push`
...
2. **How do N developers' independent local SQLite files become one team view?**
   This is a data-model question specific to how Segreant is actually built today —
   ...
   Segreant-specific half, and it's the half that determines whether "team tier" is
...
The user's framing (2026-07-10): don't host anything ourselves. Build Segreant so an
enterprise that wants a team tier can stand it up entirely on infrastructure *they*
already trust — their server, their hosting, their SSO provider — with Segreant
providing the software, never the operation.

This is a better answer than the two alternatives:

- **Segreant-hosted SaaS** — was already ruled out by the existing roadmap language
  ("requires an operator: ongoing hosting, and a support commitment"). Directly
  violates the zero-dollar-cost ceiling and turns Segreant into a company with an
...
- **BYO-server / BYO-hosting / BYO-SSO** (this proposal) — Segreant ships as
  ...
  Segreant — an enterprise's SSO credentials and server never touch anything
  Segreant operates, because Segreant operates nothing. Consistent with the
...
single-user `segreant` CLI/proxy. A small server binary the
...
an SSO issuer config (§3). Segreant's job is to make that binary correct and
...
> dependency, never pulled into the main `segreant` CLI's install. It reads
...
> and listens for pushed rollups. Unlike Segreant's own dashboard (loopback-
...
> this process speaks plain HTTP by design, matching "Segreant provides the
...
that actually determines feature size. Today, `segreant`'s entire data model is one
...
this with a command (`segreant team push`, or an opt-in background interval) that
...
`segreant pricing --refresh` already is today.
...
> domain into the other. `segreant team push` is the CLI surface: `--url`
...
not listed receives HTTP 403. Segreant deliberately does not infer groups or
...
"Bring your own SSO provider" only stays zero-maintenance if Segreant implements
...
or the `segreant team push` command touches `src/proxy/server.ts` or any
...
- **The exact rollup period/cadence.** Resolved narrowly, not fully: `segreant
