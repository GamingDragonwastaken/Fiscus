...
Segreant provides a bounded, credential-free **FOCUS v1.4 compatibility projection** for immutable provider billing evidence. It is an interoperability-shaped read model, **not FOCUS conformance** and not an invoice validator.
...
- `EffectiveCost` and `AllocatedCost` are explicitly `null`, with Segreant status fields set to `unmapped`; local estimates and allocations are never laundered into FOCUS cost metrics.
...
- `SegreantSourceLineage` retains the immutable record ID, source record ID and SHA-256, import/export IDs, source system, and operator-supplied trust label.
