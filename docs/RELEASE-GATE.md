...
| Packaged dashboard/API | Use an isolated `FISCUS_HOME` (the legacy `AEGIS_HOME` is still honoured; historical records below cite it); seed labelled demo data; start the installed dashboard; probe health, overview, value, and HTML; terminate it cleanly |
