...
| Causal-evidence integrity | Run `test/causal-core.test.ts`, `test/causal-store.test.ts`, `test/causal-cli.test.ts`, and `test/causal-dashboard.test.ts`. In a clean installed artifact, prove `fiscus causal status --json` says no registered study/no causal result by default; prove protocol preview does not write, `--apply` creates an append-only protocol/assignment record, assignment replay verifies, and a no-outcome study is `collecting`, not a causal claim. Verify `/api/causal` is GET/HEAD-only, redacts randomisation material, and does not expose an automatic routing/budget action. |
