...
| Egress disclosure | Reconcile every newly introduced outbound path with DATA-BOUNDARIES.md; distinguish Fiscus-process enforcement, proxy-routed traffic coverage, browser behaviour, and any separately validated OS/network control. Verify corrupt-history refusal, redirect `Location` stripping, and checkpoint fallback/full-scan behavior. Do not project a process-level test into a workstation-wide or provider-privacy guarantee. |
