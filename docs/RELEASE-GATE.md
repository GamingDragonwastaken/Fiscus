...
**Current internal causal substrate.** The exact branch now contains
Store-internal V2 execution, terminal-outcome, follow-up-policy,
clock-authority, qualification, and T-069 scalar lineage-validation code. These
records are not public CLI/API/dashboard evidence: the T-069 append-only
request-to-realization sidecar is not yet persisted, so cost-bearing V2
qualification remains fail-closed. The historical candidate rows below do not
cover this later source and must not be reused as exact-head release evidence.
