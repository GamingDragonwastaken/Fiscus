...
# CLI scope: broad financial-operations product, with evidence depth disclosed
# elsewhere rather than calling the whole product coding-agent-only.
replace_once(
    'src/cli.ts',
...
# The dashboard itself is local; Fiscus as a whole has explicit, purpose-scoped
# egress paths. Never collapse those two statements.
replace_once(
    'src/dashboard/web/app/main.ts',
    "h('span', { text: 'Runs on this machine only. Nothing is sent anywhere.' }),",
    "h('span', { text: 'Dashboard runs locally. Outbound actions are explicit and purpose-scoped.' }),",
)

# README opening: widen the financial scope without pretending all outcome
# modalities are equally instrumented.
replace_once(
    'README.md',
...
# The Realization Standard: the eight-gate commit funnel is the strongest coding
# adapter, not a universal ontology for every kind of AI-assisted work.
replace_once(
...
# Roadmap: refresh only rows whose old status is directly contradicted by shipped
# code. The document remains a roadmap, not a retrospective marketing claim.
replace_once(
...
# Regression test: security/scope copy is part of the trust surface.
...
test('global product copy does not collapse local dashboard execution into zero egress', () => {
  const main = read('src/dashboard/web/app/main.ts');
  assert.doesNotMatch(main, /Runs on this machine only\. Nothing is sent anywhere\./);
  assert.match(main, /Dashboard runs locally\. Outbound actions are explicit and purpose-scoped\./);
  // Keep the narrower page-load claim: the shell itself still loads no CDN,
  // analytics, font, or third-party resource.
...
print('truth-copy sweep applied')
